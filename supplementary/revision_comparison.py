...
from revision_common import ROOT,OUT,api,current_records,write_json,save_run,load_run,array_sha,row_count,record_execution
...
def run():
    before=row_count('comparison');start=time.perf_counter();ns=api();old=current_records()['outputs/data_comparison/summary.json']
            ...
            for method in LABELS:
                ...
                try:
                    if method=='A':
                        ...
                        result=run_baseline(method,source,drift,T,pot.box,
                    ...
                    row=dict(id=key,beta=beta,seed=seed,method=method,label=LABELS[method],status='ok' if good else 'failed',
                        ...
                        threshold=threshold,T=T,source_sha256=array_sha(source),target_sha256=array_sha(target),
    ...
    record_execution('comparison',time.perf_counter()-start,before,row_count('comparison'),threadpools=threadpool_info(),
        ...
        timing='Fresh fits and sampling; drift fitting included in KDE flow cost; excludes input generation and metric callbacks')
