...
def append_data_comparison(lines):
    ...
    assert config['source_x2_variance']==.3 and len(rows)==4
    lines += ['## Equal-data comparison']
    table(lines,['beta','Method','SW2','Fit seconds','Fit + sampling seconds','Reference-threshold hits'],[
        [r['beta'],r['label'],pm(dict(mean=r['sw2_mean'],std=r['sw2_std'])),
         f"{r['fit_seconds_mean']:.4f}",
    ...
    lowest_error=complete and all(lookup[beta,'A']['sw2_mean'] < min(lookup[beta,m]['sw2_mean'] for m in ['D']) for beta in config['betas'])
    lowest_cost=complete and all(lookup[beta,'A']['total_seconds_mean'] < min(lookup[beta,m]['total_seconds_mean'] for m in ['D']) for beta in config['betas'])
    claims=[]
    ...
    for beta in config['betas']:
        counts=', '.join(f"{lookup[beta,m]['label']} {lookup[beta,m]['hit_count']}/{lookup[beta,m]['n_repeats']}" for m in ['A','D'])
    ...
    terminal_counts={(r['beta'],r['method']):sum(run['sw2'] is not None and run['sw2']<next(ref['mean']+ref['std'] for ref in data['references'] if ref['beta']==r['beta'])
                     for run in data['runs'] if run['beta']==r['beta'] and run['method']==r['method']) for r in rows}
    if all(terminal_counts[r['beta'],r['method']]==r['hit_count'] for r in rows):
        lines += ['For these saved realisations, the terminal-distance counts below the threshold equal the checkpoint-crossing counts in the table. Fit time means eigenpair estimation for BKT and finite-lag drift estimation for KDE particle flow. Timings are the original single-thread measurements; no timings were remeasured for this report revision.', '']


def append_evaluation_protocol(lines):
    """Document equal cloud sizes and ratios computed from saved scalar outputs."""
    study=read_json('admissible_source/summary.json');primary=read_json('results.json')
    lines += ['### Evaluation clouds and reference-normalized errors', '',
    ...
    table(lines,['System','Transported M','Evaluation target size','Size of each reference cloud'],[
        ['2D double wells, beta=0,0.25,0.5,1',study['config']['particles'],study['config']['particles'],primary['A2_beta0.0']['reference']['configuration']['M']],
        *[[name,primary[key]['M'],primary[key]['M'],primary[key]['reference']['configuration']['M']]
          for key,name in [('B_quartic4','2D four wells'),('B_poly9','2D nine wells'),('A10_beta0.0','10D double well product, beta=0')]],
        *[[f'{d}D independent double-well product',p['config']['M'],p['config']['M'],p['reference']['configuration']['M']]
          for d in (10,50) for p in [read_json(f'product_{d}_results.json')]]])
    rows=[];same=[];independent=[]
    for beta in study['config']['betas']:
        reference=next(r['mean'] for r in study['references'] if r['beta']==beta);values=[]
        for regime,method in [('S','FD'),('S','RBF'),('I','RBF')]:
            role='main' if regime=='S' else 'coefficient_comparison'
            part=[r for r in study['runs'] if r['beta']==beta and r['regime']==regime and r['method']==method and role in r['roles']]
            assert len(part)==10 and {r['seed'] for r in part}==set(range(10))
            values.append(statistics.mean(r['sw2'] for r in part)/reference)
        rows.append([beta,*[f'{value:.6f}' for value in values]])
        same.extend(values[:2]);independent.append(values[2])
    table(lines,['beta','S: FD-201 / reference mean','S: Koopman (RBF) / reference mean','I: Koopman (RBF) / reference mean'],rows)
    lines += [f'Ratios use the unrounded ten-realisation mean error divided by the unrounded reference mean, at n=200000, r=64 and h=0.05. The observed range is {min(same):.6f}--{max(same):.6f} for S across the two spectra, and {min(independent):.6f}--{max(independent):.6f} for I with Koopman (RBF). Rounded to two decimal places, these ranges are {min(same):.2f}--{max(same):.2f} and {min(independent):.2f}--{max(independent):.2f}. The broader interval 0.64--0.74 contains the S values but is not their observed minimum and maximum.', '']
...
def write_tables():
    ...
    print('Exported current manuscript tables.')
...
def write_report():
    ...
    for name,label in [('B_quartic4','2D four-well product'),('B_poly9','2D nine wells'),('A10_beta0.0','10D double well product, beta=0'),]:
    ...
    lines+=['I uses an extra 2000 independent source draws, with coefficients fixed thereafter. Harmonic-coordinate variance 0.3 satisfies the bounded-ratio source condition for all four 2D cases; box projection and estimated eigenpairs still prevent verification of every theorem hypothesis.','']
    append_evaluation_protocol(lines)
