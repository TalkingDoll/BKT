"""B3 cost, trajectory-role rotations, iid control, and B4 path diagnostics."""
...
def variability_stage(fit_filter=None,evaluation_filter=None,seed_filter=None):
    ...
    for fit_index in range(3) if fit_filter is None else [fit_filter]:
...
def parallel_variability():
    # Fit sequentially so the three estimation costs remain isolated measurements.
    trajectories=full_trajectories();original,_=ac.inputs()
    for index,train in enumerate(trajectories):spectrum(train,index,original['record']['setting'])
    def run_one(fit_index,evaluation,seed):
        name=f'rotation_fit{fit_index}_eval{evaluation}_seed{seed}';log=OUT/(name+'.log')
        ...
        row=dict(id=name,exit_code=result.returncode,log=log.name)
        print('ROTATION FINISHED',json.dumps(row),flush=True)
        return row
    results=[]
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures=[executor.submit(run_one,i,j,s) for i in range(3) for j in range(3) if i!=j for s in SEEDS]
        for future in as_completed(futures):results.append(future.result())
    collect('alanine');write_json(OUT/'alanine_rotation_execution_children.json',results)
    if any(r['exit_code']!=0 for r in results):raise RuntimeError('A rotation process failed; all completed records and logs are retained')
...
def rbf_stage():
    saved=ae.load_npz('rbf_diagnostic.npz');record=json.loads(str(saved['record']))
    _,reference,labels,centers,k,mean,cov=ae.inputs('test');case=record['setting'];dic=ae.TensorRBF(case['grid'],case['width'])
    for name in ('collapse','control'):
        cr=record[name]
        for index,oldrow in enumerate(cr['runs']):
            seed=oldrow['seed'];identity=f'rbf_{name}_seed{seed}'
            if load_run('alanine',identity):continue
            source=ae.source_points(seed,cr['particles'],mean,cov,'iid');target=saved[name+'_target'][index]
            extra={}
            try:
                clouds,_,info=ae.integrate(source,saved['lam'],saved['basis'],saved['moments'],dic,cr['taus'],diagnostics=extra)
            except (RuntimeError,FloatingPointError) as error:
                info=extra.get('events',{});info.update(unaffected_sw2=None,
                    diagnostic_scope='Attempted RHS evaluations before failure; no completed endpoint is claimed')
                row=dict(id=identity,task='B4_rbf',case=name,seed=seed,taus=cr['taus'],endpoint=None,checkpoints=[],
                         diagnostics=info,status='failed',failure=str(error),
                         parity=dict(passes_1e12=False,reason='The prescribed re-integration did not complete; endpoint reproduction is unavailable'))
                save_run('alanine',identity,row,{'mask_'+k:v for k,v in extra.get('path_masks',{}).items()})
                collect('alanine');print('ALANINE RBF FAILED',identity,str(error),flush=True)
                continue
            masks=extra['path_masks'];affected=masks['affected']
            scores=[ae.score(x,target,labels,k,ae.thickness(target)) for x in clouds]
            info['unaffected_sw2']=metric_unequal(clouds[-1][~affected],target) if (~affected).any() else None
            checks=parity(clouds,saved[name+'_paths'][index])
            checks['periodic_max_absolute_difference']=float(np.max(abs(ae.wrap(clouds-saved[name+'_paths'][index]))))
            row=dict(id=identity,task='B4_rbf',case=name,seed=seed,taus=cr['taus'],endpoint=scores[-1],checkpoints=scores,
                     diagnostics=info,parity=checks,status='ok')
            save_run('alanine',identity,row,dict(clouds=clouds,**{'mask_'+k:v for k,v in masks.items()}));collect('alanine')
            print('ALANINE RBF',identity,row['parity'],flush=True)

def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('stage',choices=['cost','variability','iid','rbf'])
    p.add_argument('--fit-trajectory',type=int,choices=range(3));p.add_argument('--evaluation-trajectory',type=int,choices=range(3))
    ...
    with threadpool_limits(limits=1):
        ...
        {'cost':cost_stage,'variability':variability,'iid':iid_stage,'rbf':rbf_stage}[args.stage]()
        after=int(load_run('alanine',identity) is not None) if filtered else row_count('alanine')
        execution='execution_attempts/alanine_'+identity if filtered else 'alanine_'+args.stage
        record_execution(execution,time.perf_counter()-start,before,after,threadpools=threadpool_info(),
                         repetition_processes=4 if args.stage=='variability' and not filtered else 1,
                         estimation_policy='Spectra fitted sequentially; independent role-rotation transports may run in four single-BLAS-thread processes')
