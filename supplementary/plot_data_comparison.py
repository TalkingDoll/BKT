...
METHODS = ("A", "D")
COLORS = {"A": "#0072B2", "D": "#525252"}
MARKERS = {"A": "o", "D": "D"}
AXIS_LABELS = {"A": "BKT", "D": "KDE\nflow"}
...
def load_summary(path):
    ...
    if len(rows) != expected or len(lookup) != expected or any((beta, method) not in lookup for beta in betas for method in METHODS):
        raise ValueError("Expected one summary row for each of two methods and two coupling values")
...
def shared_legend(fig, axes, labels):
    ...
    legend = fig.legend([entries[label] for label in order], order,
                        ...
                        bbox_transform=fig.transFigure, ncol=len(order), frameon=False,
