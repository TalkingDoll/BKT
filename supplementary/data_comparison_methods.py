"""Data-only finite-lag drift and exact KDE particle flow.
...
scaled gradient regression with ridge 1e-8; RK4 step .05; full
...
def run_baseline(method, X0, drift: Callable, T, box, dt=None,
                 checkpoints=None, checkpoint_callback=None,
                 block_size=256):
    """Run D: estimated drift minus the exact current KDE score.
    ...
    if method != "D":
        raise ValueError("method must be 'D'.")
    ...
    dt = .05 if dt is None else float(dt)
    if T <= 0 or dt <= 0:
    ...
    return dict(method=method,status="failed" if failure else "completed",failure=failure,
                ...
                requested_steps=nsteps,completed_steps=completed,
                time_convention="int(round(T/dt)) full steps; no fractional remainder step",
                initial_particles_clipped=False,
                method_details="learned finite-lag drift minus exact current-cloud Gaussian KDE score; full covariance recomputed at every RK4 stage; no speed cap")
