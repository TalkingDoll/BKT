"""Shared diagnostics and reproducible execution support for the retained experiments."""
...
def definitions(ns,cells):
    path=ROOT/'BKT_experiments.ipynb'
...
def api():
    ...
    # Use the atomic content-addressed cache writer from the notebook.
    notebook=json.loads(path.read_text(encoding='utf-8'))
...
def current_records():
    """Read the current published configurations and reference statistics."""
    from manuscript_results import read_json
    names=('results.json','ou_results.json','ou_hd_results.json','ou_hd_uncoupled_results.json',
           'product_10_results.json','product_50_results.json',
           'admissible_source/summary.json','data_comparison/summary.json')
    return {'outputs/'+name:read_json(name) for name in names}
...
def record_execution(stage_name,wall_seconds,before,after,**metadata):
    ...
    if previous is not None and after==before:return
    total=(previous.get('wall_seconds',0.) if previous else 0.)+wall_seconds
    write_json(path,dict(**metadata,wall_seconds=total,
                         timing_scope='Measured work producing the retained prescribed records'))
