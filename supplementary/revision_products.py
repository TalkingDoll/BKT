...
from revision_common import OUT,api,definitions,current_records,load_run,save_run,write_json,array_sha,row_count,record_execution
from revision_synthetic import observe,collect
from revision_common import PRODUCT_SEED_INDICES,PRODUCT_TARGET_SEED

def setup():
    ns=definitions(api(),[18]);G=ns['setup_1d'](lambda x:(x**2-1)**2)
...
def run(d,seed_index=None):
    ns=setup();original=current_records()[f'outputs/product_{d}_results.json']
    ...
    for seed in PRODUCT_SEED_INDICES if seed_index is None else [seed_index]:
        ...
        for name in names:
            ...
            sampling_seconds=time.perf_counter()-tic
            ...
            row=dict(id=identity,system=f'product{d}',dimension=d,seed=seed,method={'exact':'FD','estimated':'RBF','legendre':'Legendre'}[name],
                ...
                sw2=metrics['sw'],metrics=metrics,reference=original['reference']['metrics']['sw'],diagnostics=info,status='ok',
            ...
            print('PRODUCT',identity,'SW2',row['sw2'],'seconds',sampling_seconds,flush=True)
...
def main():
    ...
    with threadpool_limits(limits=1):
        ...
        if args.seed_index is None:
            record_execution(f'product{args.dimension}',time.perf_counter()-start,before,row_count('products'),coordinate_workers=8,threadpools=threadpool_info(),
                             cost_interpretation='Eight independent coordinate workers; descriptive cost, not part of the single-thread sampler comparison')
