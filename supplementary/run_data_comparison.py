...
LABELS = {"A": "BKT", "D": "KDE particle flow"}
...
def run(config_path, resume=False, probe=False, output_directory=None):
    ...
    assert config["drift_ridge_after_column_scaling"] == 1e-8
    ...
    def persist():
        ...
        partial["timing_provenance"] = dict(per_run_status="measured", fit_policy="Fresh single-thread fits; drift fitting included in KDE flow cost")
    ...
    result["runs"] = rows
    result["timing_provenance"] = dict(per_run_status="measured", fit_policy="Fresh single-thread fits; drift fitting included in KDE flow cost")
