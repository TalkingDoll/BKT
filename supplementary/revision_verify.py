...
import json
import numpy as np
from revision_common import ROOT,OUT,api,definitions,load_run,empirical_sw2,write_json,baseline_records
...
def main():
    ...
    for folder in ('a2','a10','multi','ou','products','comparison','alanine'):
        ...
        for row in rows:
            ...
            if folder=='alanine':
                ...
                if row.get('task')=='B4_rbf':
                    data=ae.load_npz('rbf_diagnostic.npz');record=json.loads(str(data['record']))
                    i=[r['seed'] for r in record[row['case']]['runs']].index(row['seed'])
                    target=data[row['case']+'_target'][i];x=arrays['clouds'][-1];value=row['endpoint']['sw2']
                else:
    ...
    result=dict(tasks=report,checked_masks=mask_count,max_absolute_metric_difference=maximum,
                scope='Recomputed full/filtered distances and all mask unions, counts and fractions; baseline reproduction discrepancies remain separately reported')
