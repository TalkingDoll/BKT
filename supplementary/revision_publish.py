...
import numpy as np
from revision_common import ROOT,OUT,api,baseline_records,write_json,load_run,save_run,array_sha,summarize,PRODUCT_SEED_INDICES
...
def snapshot_sidecars():
    path=OUT/'baseline_sidecars.json'
    if path.exists():return json.loads(path.read_text())
    names=['admissible_source/'+n for n in ('probe_checks.json','validation_checks.json',
            'metadata_beta0.json','metadata_beta0.25.json','metadata_beta0.5.json','metadata_beta1.json')]
    names += ['data_comparison/'+n for n in ('input_probe.json','input_provenance.json',
            'numerical_provenance.json','validation_checks.json','endpoint_diagnostics.json')]
    names += [p.relative_to(ROOT/'outputs').as_posix() for p in sorted((ROOT/'outputs/data_comparison').glob('drift_*.json'))]
    saved={name:json.loads((ROOT/'outputs'/name).read_text()) for name in names}
    write_json(path,saved)
    return saved


def refresh_sidecars(a2,comparison,baseline):
    """Replace current audit metadata without losing the historical records."""
    ...
    for name,rows,task in [('admissible_source',a2,'a2'),('data_comparison',comparison,'comparison')]:
        checks=[dict(id=r['id'],**r['parity']) for r in rows if r.get('parity')]
        write_json(ROOT/'outputs'/name/'validation_checks.json',dict(
            ...
            scope='Saved full/filtered distances and masks recomputed by revision_verify; original endpoint checks retained per realisation. This is not a refit or sampler rerun.',
            ...
            original_endpoint_checks=checks,
            historical_audit='outputs/revision/baseline_sidecars.json'))
    ...
    write_json(ROOT/'outputs/admissible_source/probe_checks.json',dict(
        ...
        scope='First-coordinate source equality for all ten S and I designs; prior protocol checks are preserved in the baseline sidecar snapshot.',
        historical_audit='outputs/revision/baseline_sidecars.json'))
    ...
    previous={(r['beta'],r['seed']):r for r in baseline['data_comparison/input_probe.json']}
    for r in provenance:
        old=previous.get((r['beta'],r['seed']))
        if old:
            keys=('training_x_sha256','training_y_sha256','source_sha256','target_sha256','directions_sha256')
            assert all(r[k]==old[k] for k in keys)
        r.update(source_matches_main=True,fixed_inputs_match_previous=True if old else None,
                 scope='Current ten-seed input hashes; no load-time measurement is implied')
    ...
    write_json(folder/'numerical_provenance.json',dict(
        ...
        source_snapshots='outputs/revision/execution_manifest.json',
        per_realisation_records='outputs/revision/comparison_runs.json',
        historical_audit='outputs/revision/baseline_sidecars.json',
        legacy_drift_file_scope='The retained drift_*.npz and drift_*.json files belong to the original five-seed diagnostic. Fresh ten-seed fit and transport timings are stored in runs.json; models can be refitted from the preserved training data.'))
    for path in sorted(folder.glob('drift_*.json')):
        old=baseline[path.relative_to(ROOT/'outputs').as_posix()]
        write_json(path,dict(old,revision_scope='Historical five-seed fit diagnostic; its elapsed fields are not the fresh B2 costs. Use current runs.json for fit and sampling times.'))
...
def publish_regular(primary,folder):
    ...
    for name,part in grouped(rows,['system']).items():
        ...
        case['timing_record']=f'outputs/revision/{folder}_execution.json'
        ...
        primary[name]=case
    if folder=='a10':
        old=baseline_records()['outputs/a10_sweep.json'];scan=copy.deepcopy(old)
        scan['config'].update(selection='Largest absolute moments from 100000 independent source points, seed 900; velocity coefficients still use seed 500',selection_sample_count=100000,selection_seed=900)
        scan['rows']=[dict(r,dictionary={'RBF':'rbf','Legendre':'poly'}[r['method']],size=r['dictionary_size_parameter']) for r in rows if r.get('role')=='dictionary_rank_scan']
        assert len(scan['rows'])==12
        write_json(ROOT/'outputs/a10_sweep.json',scan)
    return rows
...
def publish_a2(primary=None):
    ...
    old=baseline_records()['outputs/admissible_source/summary.json'];folder=ROOT/'outputs/admissible_source';allrows=[]
    for row in rows:
...
def publish_comparison():
    rows=source_rows('comparison');old=baseline_records()['outputs/data_comparison/summary.json']
    ...
    assert {(r['beta'],r['seed'],r['method']) for r in rows}=={(b,s,m) for b in (0.,.5) for s in range(10) for m in ('A','B','D')}
    ...
    for beta in cfg['betas']:
        for seed in cfg['source_indices']:
            pair=[r for r in rows if r['beta']==beta and r['seed']==seed and r['method'] in ('B','D')]
            assert len(pair)==2
            if all(r['status']=='ok' for r in pair):
                assert pair[0]['fit_seconds']==pair[1]['fit_seconds'],(beta,seed,'shared drift-fit cost')
...
def publish_ou():
    rows=source_rows('ou');base=baseline_records()
...
def publish_products():
    rows=source_rows('products');base=baseline_records()
    ...
    for d in (10,50):
        ...
        result['config']['seed_correction']='Source seeds are 1+s, skipping target seed 7. Index 6 is excluded from current tables and figures; index 10 (source 11, training 1011) replaces it. The excluded record remains an explicitly historical audit input.'
...
def main():
    ...
    primary=copy.deepcopy(baseline_records()['outputs/results.json'])
    ...
    from revision_followup import export_public
    ...
    if summary.get('followup'):
        write_json(ROOT/'configs/retained/revision_followup.json',summary['followup']['configuration'])
        manifest['files']['configs/retained/revision_followup.json']=dict(archive='double_well.zip',member='outputs/revision/followup_config.json')
    ...
    execution_path=OUT/'execution_manifest.json'
    if execution_path.exists():
        execution=json.loads(execution_path.read_text())
        execution['complete_definition']='All runner processes finished without programming errors; individual numerical runs may be incomplete or failed and remain in the data.'
        execution['published_source_sha256']={p.relative_to(ROOT).as_posix():hashlib.sha256(p.read_bytes()).hexdigest()
            for p in [ROOT/'BKT_experiments.ipynb',ROOT/'requirements.txt',*sorted((ROOT/'supplementary').glob('*.py'))]}
        execution['source_snapshot_scope']='implementation_sha256 is the queue-start snapshot. published_source_sha256 records the final code, including reporting, execution-entry and diagnostic error-reporting refinements made during the run. Numerical settings are recorded separately in configuration.json.'
