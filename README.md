...
The fixed experiment protocol is run by `supplementary/revision_queue.py`, followed
...
mode selection, ten-repetition summaries, fixed-fit alanine evaluation, and
per-particle safeguard diagnostics. The report contains the retained results
and saved-array verification.

Alanine uses the first trajectory for spectrum fitting and source construction.
All three source seeds are retained. The two evaluation trajectories share this
fixed fit; their variation does not measure robustness across fitted spectra.
The single results report contains the final configurations and results,
without the exploratory runs or revision history.
